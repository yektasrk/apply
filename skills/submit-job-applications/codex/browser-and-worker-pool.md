...
3. Keep exactly one live application tab per occupied worker. Put it in an expanded one-tab group named `Apply — <Role> — <Worker> — R<n>`; use the actual worker name or a stable fallback such as `Agent 1`. Keep every active group visible in the same Chrome window.
