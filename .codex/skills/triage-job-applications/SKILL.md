...
Codex writes the suitability reasoning, rejection reasons, and cover-letter prose itself after reading the full job description, resume, and available performance-review evidence. Do not call `the local repository/job-apply-agent`, local scripts, external generators, automated scoring systems, or API-based generation to produce those substantive outputs unless the user explicitly reverses this requirement.
...
`the local repository/cover_letters/Denmark/Kamstrup.md`
